 ...
 */
public interface EditPartViewer extends org.eclipse.jface.viewers.ISelectionProvider {
	...
	/**
	 * Adds a <code>TransferDragSourceListener</code> to this viewer. This has the
	...
	/**
	 * Adds a <code>TransferDropTargetListener</code> to this viewer. This has the
	...
	/**
	 * Removes the specified <code>EditPart</code> from the current selection. If
	 * the selection becomes empty, the viewer's {@link #getContents() contents}
	 * becomes the current selected part. The last EditPart in the new selection is
	 * made {@link EditPart#SELECTED_PRIMARY primary}.
	 * <P>
	...
	/**
	 * Deselects all EditParts. The viewer's {@link #getContents() contents} becomes
	 * the current selection. Fires selection changed to
	 * {@link org.eclipse.jface.viewers.ISelectionChangedListener}s.
	 */
	void deselectAll();

	/**
	 * Returns <code>null</code> or the <code>EditPart</code> associated with the
	 * specified location. The location is relative to the client area of the
	 * Viewer's <code>Control</code>. An EditPart is not directly visible. It is
	 ...
	 * associated with the top-level model object. It is considered to be "The
	 * Diagram". If the user has nothing selected, the <i>contents</i> is implicitly
	 * the selected object.
	 * <P>
	 ...
	 * register themselves with multiple keys. The purpose of such registration is
	 * to allow an EditPart to be found by other EditParts, or by listeners of
	 * domain notifiers. By default, EditParts are registered by their model.
	 * <P>
	...
	/**
	 * Returns the <code>KeyHandler</code> for this viewer. The KeyHandler is sent
	 * KeyEvents by the currently active <code>Tool</code>. This is important,
	 * because only the current tool knows if it is in a state in which keys should
	 ...
	 * viewer has a Control, clients may access the viewer's resource manager. Any
	 * resources constructed using this manager, but not freed, will be freed when
	 * the viewer's control is disposed. This does not mean that clients should be
	 * lazy about deallocating resources. If a resource is no longer needed but the
	 * viewer is still in use, the client must deallocate the resource.
	 * <P>
	 * Typical usage is by EditParts contained inside the viewer. EditParts which
	 * are removed from the viewer should free their resources during
	 * {@link EditPart#removeNotify()}. When the viewer is disposed,
	 * <code>removeNotify()</code> is not called, but the viewer's resource manager
	 * will be disposed anyway.
	 * <P>
	 ...
	 * that serves as the parent to the contents editpart. The <i>root</i> is never
	 * selected. The root does not correspond to anything in the model. The User
	 * does not interact with the root.
	 * <P>
	...
	/**
	 * Returns an unmodifiable <code>List</code> containing zero or more selected
	 * editparts. This list may be empty. In contrast, the inherited method
	 * {@link org.eclipse.jface.viewers.ISelectionProvider#getSelection()} should
	 ...
	 * {@link org.eclipse.jface.viewers.ISelectionProvider ISelectionProvider}. This
	...
	/**
	 * Returns the viewer's selection manager. The selection manager has complete
	...
	/**
	 * Sets a property on this viewer. A viewer property is an arbitrary key-value
